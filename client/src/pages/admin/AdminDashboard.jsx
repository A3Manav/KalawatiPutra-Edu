		...
		const fetchData = async () => {
			try {
				const [
					...
					axios.get(`${VITE_API_URL}/admin/roadmaps`),
					axios.get(`${VITE_API_URL}/admin/colleges`),
					axios.get(`${VITE_API_URL}/admin/counseling`),
					axios.get(`${VITE_API_URL}/admin/dsapractice`),
					axios.get(`${VITE_API_URL}/admin/internships`),
					axios.get(`${VITE_API_URL}/admin/mentorships`),
				]);
				setData({
				...
				return (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{[
							...
							<div
								key={item.id}
								className="bg-black bg-opacity-70 p-6 rounded-lg shadow-card backdrop-blur-sm hover:bg-emerald-900 hover:bg-opacity-20 transform transition-all duration-300 hover:scale-[1.02] cursor-pointer"
								onClick={() => setActiveTab(item.id)}
							>
								<div className="flex items-center justify-between">
									<h3 className="text-xl font-medium text-emerald-400 group-hover:text-emerald-300">
										{item.name}
									</h3>
									{item.icon && (
										<svg
								...
								<div className="absolute inset-0 border border-emerald-500 rounded-lg opacity-0 hover:opacity-100 transition-opacity duration-300"></div>
							</div>
						))}
						<Link
							to="/admin/manage-articles"
							className="bg-black bg-opacity-70 p-6 rounded-lg shadow-card backdrop-blur-sm hover:bg-emerald-900 hover:bg-opacity-20 transform transition-all duration-300 hover:scale-[1.02] relative group"
						>
							<h3 className="text-xl font-medium text-emerald-400 group-hover:text-emerald-300">
								Manage Articles
							</h3>
							<p className="mt-2 text-emerald-200">
								Create, edit, approve, or delete articles
							</p>
						...
						<Link
							to="/admin/contact-message-management"
							className="bg-black bg-opacity-70 p-6 rounded-lg shadow-card backdrop-blur-sm hover:bg-emerald-900 hover:bg-opacity-20 transform transition-all duration-300 hover:scale-[1.02] relative group"
						>
							<h3 className="text-xl font-medium text-emerald-400 group-hover:text-emerald-300">
								Contact Messages
							...
							<p className="mt-2 text-emerald-200">
								Read or delete contact messages
							</p>
							<div className="absolute inset-0 border border-emerald-500 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
						</Link>
						<Link
							to="/admin/manage-goodies"
							className="bg-black bg-opacity-70 p-6 rounded-lg shadow-card backdrop-blur-sm hover:bg-emerald-900 hover:bg-opacity-20 transform transition-all duration-300 hover:scale-[1.02] relative group"
						>
							<h3 className="text-xl font-medium text-emerald-400 group-hover:text-emerald-300">
								Manage Goodies
			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="mb-8 border-b border-gray-800">
					<nav className="flex space-x-4 overflow-x-auto pb-1">
						<button
							className={`emerald-nav-item px-3 py-2 text-sm font-medium ${
								activeTab === "dashboard"
						...
						<button
							className={`emerald-nav-item px-3 py-2 text-sm font-medium flex items-center gap-2 ${
								activeTab === "dsapractice"
									? "text-emerald-400 active"
									: "text-gray-400"
							}`}
						...
						<button
							className={`emerald-nav-item px-3 py-2 text-sm font-medium ${
								activeTab === "workshop_certificate"
									? "text-emerald-400 active"
									: "text-gray-400"
							}`}

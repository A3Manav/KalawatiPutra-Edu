...
const Cart = () => {
	...
	const removeItem = async (goodieId) => {
		try {
			await axios.delete(`${VITE_API_URL}/cart/remove/${goodieId}`, {
				headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
			});
	...
	const handleCheckout = async () => {
		...
		try {
			...
			setError("Checkout failed. Please try again. or contact to administrator.");
	...
	const updateQuantity = async (goodieId, newQuantity) => {
		...
		try {
			await axios.put(
				`${VITE_API_URL}/cart/update/${goodieId}`,
			...
			<div className="relative z-10">
				...
				<header className="backdrop-blur-md bg-black/40 border-b border-purple-800/50">
					<div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
						...
						<button
							onClick={() => navigate(-1)}
							className="text-emerald-400 hover:text-teal-300 font-medium"
						>
							Continue Shopping
						</button>
					</div>
